...
export type EditorView = 'editor' | 'worldbuilding' | 'cards'
...
export function Editor() {
  ...
  const toggleThreadPanel = (threadId: string) => {
    setOpenThreadIds((ids) => {
      ...
      if (ids.length >= 2) return [...ids.slice(1), threadId]
      return [...ids, threadId]
    })
  }

  const createPlyCell = async (data: any) => {
    if (!activeDoc) return
    const cell = await window.weave.plyCells.create({ ...data, documentId: activeDoc.id })
    setPlyCells((c) => [...c, cell])
  }

  const updatePlyCell = async (id: string, data: any) => {
    ...
    <div className={styles.editor}>
      <div className={styles.titlebar}>
        ...
        <div className={styles.tabs}>
          {(['editor', 'cards', 'worldbuilding'] as EditorView[]).map((v) => (
      ...
      <div className={styles.body}>
        {view === 'editor' && (
          <>
            <EditorSidebar
              documents={documents} activeDocId={activeDoc?.id}
              ...
              threads={threads} openThreadIds={openThreadIds}
              onToggleThread={toggleThreadPanel}
              projectId={projectId!} onThreadsChange={setThreads}
            />
              ...
              {activeDoc ? (
                <div className={styles.editorRow}>
                  <div className={styles.textEditorWrap} style={{ width: editorWidth }}>
                    <TextEditor
                      document={activeDoc} plyCells={plyCells} threads={threads}
                      openThreadIds={openThreadIds} onContentChange={updateDocContent}
                      onCreatePlyCell={createPlyCell} onToggleThread={toggleThreadPanel}
                    />
                  </div>
        ...
        {view === 'cards' && projectId && (
          <CardBoard projectId={projectId} threads={threads} plyCells={plyCells} />
        )}

        {view === 'worldbuilding' && (
          <div className={styles.editorArea}>
            <div className={styles.noDoc}><div className={styles.noDocText}>worldbuilding — coming soon</div></div>
          </div>
        )}
      </div>
    </div>
